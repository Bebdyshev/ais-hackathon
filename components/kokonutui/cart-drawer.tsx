        ...
        <div className="flex flex-col h-full">
            ...
            {cart.map((item) => (
              <div key={item.id} className="flex gap-4 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded-lg">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-base font-medium mt-1">{item.price * item.quantity} points</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="p-4 border-t border-zinc-200 dark:border-zinc-800">
            <div className="flex justify-between mb-4">
              ...
              <span className="text-base font-medium">{total} points</span>
            </div>
            <div className="flex justify-between mb-4">
              <span className="text-sm text-zinc-500 dark:text-zinc-400">Your Points</span>
              <span className="text-sm font-medium">{points} points</span>
            </div>
            <button 
              onClick={onPurchase}
              disabled={cart.length === 0 || total > points}
              className={`w-full py-3 text-base font-medium rounded-lg transition-colors ${
                cart.length === 0 || total > points
                  ? "bg-zinc-200 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 cursor-not-allowed"
                  : "bg-green-600 text-white hover:bg-green-700"
              }`}
            >
              {total > points ? "Not Enough Points" : "Purchase"}
            </button>
          </div>
        </div>
      </motion.div>
